...
[ TODO - make "solving the safe" into a test command ]
[ TODO - revelation from birth/death records ]
[ TODO - add atmospheric effects back in ]
[ TODO - weird scenes in Gallery painting ]
[ TODO - implement a keyring using Locksmith ]
[ TODO - implement the drunk man ]
[ TODO - implement end of Day 2 ]
[ TODO - implement laptop ]
[ TODO - implement secret passage in Study ]
[ TODO - implement fish oil ]
[ TODO - implement the hatch in Bare Foundations ]
...
The player is in Drecords.

Book 2 - Extensions

The player has a number called day. Day is initially 1.
The player has a number called spider. Spider is initially 0.
The player has a number called births. Birth is initially 0.
The player has a number called deaths. Deaths is initially 0.

Every thing has a number called last turn examined. The last turn examined is initially -2.
...
Instead of sleeping:
	if the location is not Master Bedroom:
		say "This isn't the best place for it.";
	if the player is not on the bed:
...
Instead of going south from the Town Square during Day 1, say "The courthouse is closed for the day.".
Instead of going south from the Asylum Courtyard during Day 1, say "The asylum is closed for the day.".
...
Instead of saying hello to Michael during Michael Reads:
	if Michael is not greeted:
		say "Michael looks up, startled, then realizes it's you and smiles. 'Hi, hon,' he says, closing the book and stretching. Then he glances at his watch. 'Good grief, what happened to the time? I'm so sorry -- I just lost track.' He smiles sheepishly.
...
Instead of asking Michael about during Michael Reads, try saying hello to Michael. Instead of showing something to Michael during Michael Reads:
	if Michael is greeted:
		if the noun is the house key or the noun is the cellar key:
			say "'Great!' says Michael. 'You hang on to that. Just a second, I'll put this back.' He takes the book up to the circulation desk and hands it to the librarian. 'Okay then,' he says, returning, 'let's
go.'
...
Michael Sleeps is a scene. Michael Sleeps begins when the goal of Michael is sleep. Michael Sleeps ends when Day 1 ends.
...
Day 2 is a scene. Day 2 begins when the day of player is 2.

When Day 2 begins:
	say "Gratefully, you remove your clothes, leave your things on the bedroom floor, crawl into bed and pull the covers up to your chin.
	
Almost immediately you fall asleep, and as the wind slithers around the eaves and the house creaks
furtive secrets to itself, you drift into troubling dreams...";
	pause the game;
	say "[the day2 intro]";
	pause the game;
	display the boxed quotation "It is hard to prevent the impression         
...
The Alley is a room. "This narrow aperture between two buildings is nearly blocked with piles of rotting cardboard boxes and overstuffed garbage cans. Ugly, half-crumbling brick walls to either side totter oppressively over you. The alley ends here at a tall, wooden fence.

High up on the wall of the northern building there is a narrow, transom-style window. [if the cans are moved]One of the garbage cans has been pushed up against the wall directly underneath it.[end if]". The Alley is southeast from Office Exterior.
...
Before opening the window:
	if the player is not on the cans:
		instead say "The window is too high.";
	otherwise if the cans are not moved:
		instead say "Hmm. You still can't quite reach, because the garbage can is too far away from the wall. Perhaps if you pushed it closer...".
...
Narrow Beach is a room. "This narrow strip of beach is tucked away between two outcroppings in the predominantly rocky shoreline, accessible only from a steep, muddy slope to the west. The sand is filthy and strewn with rocks, seaweed, litter and other bits of storm-tossed detritus.
...
The styrofoam cup is a thing in Office Interior. It is not fixed in place. "Someone seems to have left a cup of coffee sitting out, half-finished and cold." The description of the styrofoam cup is "In the styrofoam cup is some cold, murky coffee.". The coffee is part of the cup. The description of the coffee is "Muddy-looking and unappetizing. Who knows how long it's been sitting out?". Before drinking the coffee:
...
Before examining the flask, now the flask is examined. After taking the flask:
	if the flask is not examined:
		say "Taken. [the description of the flask].";
		now the flask is examined;
		rule succeeds;
	otherwise:
		continue the action.

Before going to the Local Pub:
	now the flask is flagged.

Before going south from the Local Pub:
	if the location of the flask is nowhere:
		if the flask is flagged:
			now the flask is not flagged;
			instead say "As you make your way to the door, your foot strikes something under one of the tables.";
	continue the action.
...
After asking the proprietor about:
	if the topic understood includes "amulet":
		if the amulet is in the display case:
			say "The proprietor reaches in through the back of the case and takes out the amulet. It spins slowly as he holds it up to the light.
...
'And there you have it,' he says. 'A present for you.' He hands it back to you.";
...
The rlamps are scenery in the Library. Understand "reading" and "lamps" as the rlamps. "The lamps have curved, brass stands and frosted green shades; the kind of thing you'd expect to see on an accountant's desk in the 1930s.".

The desks are scenery in the Library. "The desks are small but comfortable." Understand "reading desks" as the desks.

Michael is in the Library.

[ Circulation Desk ]

The Circulation Desk is a room. "A high counter divides the public portion of the library from the reserved stacks -- all the more esoteric and mysterious volumes from the university's collection. If anything, the area behind the counter is even more shadowy than the side you're on. The main reading area lies south.

Hanging on the wall next to the counter is a small, printed sign. Beneath the sign sits a heavy, dog-eared register, and sitting next to the register is a bell." Instead of going nowhere from the Circulation Desk, say "The alcove only opens to the south.". The Circulation Desk is north of the Library.
...
The register is scenery in the Circulation Desk. "It's basically a log, recording what book was checked out, who checked it out, and when. There must be hundreds of names here.". Check taking the register: say "That's library property." instead.
...
The librarian is a woman. "The librarian stares at you silently." The description of the librarian is "Her slack, pallid skin looks as if it has seen little more than the inside of a cave for most of her life. She stares at you silently with disturbingly large and watery eyes.".

The bell is scenery in the Circulation Desk. "One of those little round bells with the button on top; you ring it for service.". Check taking the bell: say "That's library property." instead. Check ringing the bell:
...
Before going south from the Circulation Desk:
	if the librarian is visible:
		say "The librarian fades silently into the darkness.";
	now the librarian is nowhere.

[
Report showing Michael's faculty card to the librarian:
    say "The librarian looks the card over, nods slowly, and hands it back to you without a word.";
    now the librarian is open.

After asking the librarian for the book:
    if the librarian is not open:
    ...
    otherwise:
        ...
        if frazer is nowhere:
            increase the score by 2;
        now the player has frazer;
        now the librarian is nowhere.
]

Frazer is a thing with printed name "[italic type]A Historical Overview of Superstitions[roman type] by J. Arnsworth Frazer". The description of Frazer is "A thick and weighty tome, its full title is [italic type]A Historical Overview of Superstitions in the Miskaton Valley Region[roman type] by J. Arnsworth Frazer, published in 1906. It begins: 'Although New England has always been an abundant storehouse of American myth and folklore, the Miskaton River Valley has long been recognized as particularly fecund ground for tall tales and fanciful superstition. Legends abound of hideous, inhuman races living within the venerable hills; of pagan rituals enacted at unholy burial grounds and dedicated to ancient, blasphemous gods...' and continues along the same lines in the typically dry and bombastic style of those times.". The player is holding Frazer.
...
[ TODO - on down or north: You pass through the gate and down the stone steps, which curve northwest as they lead down to the edge of the river. ]
...
[ TODO - go southeast to go up the stairs ]
...
The small metal door is a door. "A small metal door, blotchy with rust, is set into the south bank of the river, underneath the bridge.". It is south from Under the Bridge.
...
text	result
"verlac"	"There are dozens of Verlacs listed here; you'll have to be more specific than that"
"wilhelm"	"Wilhelm Verlac: born March 11, 1651"
"heinrich"	"Heinrich Verlac: born October 18, 1734"
"elijah"	"Elijah Verlac: born December 16, 1802"
"mordecai"	"Mordecai Verlac: born January 30, 1886"
"edward"	"Edward Verlac: born April 26, 1960"
...
The birth records are scenery in Brecords. Understand "documents" as the birth records. "The records go all the way back to the seventeenth century and are quite extensive. It would take a while to sort through all the papers, but you could look an individual up by name. Although it would take longer, you can also flip through all the documents in a given year to see if any familiar names pop up.". Instead of consulting the birth records about:
	repeat through Table of Birth Records:
		if the topic understood matches text entry:
			say “[result entry].”;
			now the births of player is the births of player + 1;
[			if the births of player is greater than 2 and the deaths of player is greater than 2:
				pass;
...
Table of Death Records
topic	result
"verlac"	"There are dozens of Verlacs listed here; you'll have to be more specific than that"
"wilhelm"	"Wilhelm Verlac: died October 18, 1734"
"heinrich"	"Heinrich Verlac: died December 16, 1802"
"elijah"	"Elijah Verlac: died January 30, 1886"
"mordecai"	"Mordecai Verlac: died April 26, 1960"
"edward"	"Edward Verlac: died March 14, 1997"
...
The death records are scenery in Drecords. Understand "documents" as the death records. "The records go all the way back to the seventeenth century and are quite extensive. It would take a while to sort through all the papers, but you could look an individual up by name. Although it would take longer, you can also flip through all the documents in a given year to see if any familiar names pop up.". Instead of consulting the death records about:
	repeat through Table of Death Records:
...
Interior Rooms is a region. Dining Room, Kitchen, Back Hall, Gallery, Sitting Room, Upstairs Hallway, Kid's Room, Study and Attic are in Interior Rooms.

Book 1 - Front of the House

[ Outside the House ]

Outside the House is a room. "The lane runs up from the southeast and ends at a wide clearing surrounded by gnarled and ancient trees. A wide, curving driveway runs up to the front door of your house, which lies north.

[if unvisited]The fabled Verlac family mansion looms before you in the gloom, its dark creaking presence dominating the clearing and, somehow, even though it is not visible through the trees, the entire valley. The foreboding shadow of the Verlacs seems to enshroud all of Anchorhead from here.[otherwise]The Verlac mansion looms before you, casting an air of menace ove the clearing.[end if]". It is northwest from Scenic View.
...
The front door is scenery and a lockable locked door. The description of the front door is "The front door is a huge, paneled slab of black oak adorned with a brass knocker in its center." It is north of Outside the House and south of Foyer. Instead of unlocking keylessly the front door, if the player has the house key, try unlocking the front door with the house key. Instead of locking keylessly the front door, if the player has the house key, try locking the front door with the house key.
...
The Foyer is a room. "Although it appears spacious from the outside, the house's interior feels cramped and gloomy. The walls seem too close together; the ceiling is too high. The doorways, leading in several directions, are narrow and filled with shadows, and the stairs leading up to the second floor are steep and rickety. This is not a house that makes you feel welcome. It is a house that makes you feel tiny and timid, and afraid of dark places. It is a house that makes you feel alone.[If the front door is open]

The front door stands open to the south.[end if][if unvisited]
...
Night has now undeniably fallen, and the house is very, very dark. There is probably just enough residual ambience to feel your way upstairs to the bedroom, but the rest of the house is a tenebrous maze of shadows, and any exploring would probably best be done in the morning.[otherwise]

Your luggage is still here, spread out all over the foyer.[end if]".
...
The album is a thing in the Sitting Room. "On the sofa is a family album.". The description of the album is "It's a slim, hardbound volume in dark leather, unadorned except for name 'Verlac' embossed on the
front. Glancing at the title page, you notice two details: one, the book was self-published; and
two, it was published in 1944. So it's a good bet that Edward Verlac and his family aren't mentioned
here.".

[ TODO - That's not a name you're familiar with. Since the family album is filled with names you're not
familiar with, you are unable to get very far looking for it. ]
...
Instead of rubbing the spider web:
	say "You gingerly stick your hand into the tangled spider web, and something light and skittery crawls across your skin and bites you!
...
The Wine Cellar is a room. "This wing of the cellar is even older than the rest, with walls of piled, unmortared stone. The entire room is filled with wine racks. They cover every wall except for one spot to the west, where an exit leads back to the main cellar.

All of the racks are dust-covered and empty except for one against the north wall, which contains some forty or fifty dark bottles.[if unvisited]
...
You smile a bit, recalling your husband's ability to turn a piece of insignificant trivia into a compelling story. Still, though, now that you're actually standing here... it all seems very strange.[end if]". The Wine Cellar is east of the Cellar.
...
The ladder is a thing. "A rickety wooden ladder stands here, descending from a three-foot by three-foot square of darkness in the ceiling." The ladder is fixed in place.

[ Study ]

The Study is a room with printed name "Library". "Whatever else their faults may have been, the Verlacs were evidently not ones to shun the printed word. Books line the walls from floor to ceiling in this dark-paneled, green-carpeted room, interrupted only by doorways to the east and south. A rich, brown leather armchair sits in stately repose near the window, with a polished brass pipe stand nearby completing the picture of some blue-blooded country squire's literary refuge. Once again, you are struck by how easily this place could have been the perfect home.[if the safe is in the Study]

A section of the shelves in the western wall has slid to one side, revealing a hidden safe.[end if]". The Study is north of Upstairs Hall.

The mahogany doors are a plural-named closed door. The initial appearance of the mahogany doors is "A beautiful pair of mahogany sliding doors stand closed to the east.". The description of the mahogany doors is "Each door is carved from a single piece of mahogany, and polished to a deep, rich shine. The doors are [if the mahogany doors are open]open[otherwise]closed[end if].". The mahogany doors are east from the Study.

The armchair is an enterable supporter and scenery in the Study. Understand "chair" as the armchair. "It's the kind of furniture that you always drool over in catalogs, and it looks about as comfortable as an armchair ever gets without an added shot of heroin.". Report entering the armchair:
...
Amazingly, a section of the bookshelf slides back, revealing a hidden safe.";
	...
	otherwise if the safe is nowhere:
...
The back door is a closed locked door and scenery. It is north from the Kitchen and south from the Path Behind the House. The description of the back door is "TODO".
...
William's Coffin is a closed openable container and scenery. "Unlike the others, William's coffin is a ramshackle affair of cheap pine, held together with rusting nails. It's half-rotten with age[if closed] and so shabbily built you could probably open it with your bare hands[end if].". Report opening William's Coffin:
	say "Compelled by a gruesome curiosity, you dig your fingers under the soft, rotting boards and pry up the lid. With the shriek of pulling nails and a shower of crumbling wood, the coffin comes open, and you look inside, expecting to find a child's tiny skeleton...
...
[ from Upstairs Hall ]
Test study with "e / n / x chair / x stand / take book / read book / drop book / x doors / sit on chair / stand".
...
Goal is a kind of value. Goals are ignore, follow, sleep, write.
...
[if the goal of Michael is ignore]At the moment, Michael is largely absorbed in his reading, and is paying little attention to you.[otherwise]Michael looks at you expectantly, waiting for you to lead the way.[end if]".
...
Volume 11 - Simple Things

Book 1 - My Things

The clothes are a thing. The indefinite article of the clothes is "your". The description of the clothes is "A tasteful ensemble from Ann Taylor in muted browns and olive greens (you've been an autumn person since you were old enough to match your own socks).". The clothes are worn. After taking the clothes, try wearing the clothes. Check taking off the clothes:
	if the location of the player is the Master Bedroom or the location of the player is the Bathroom:
		rule succeeds;
	otherwise:
		 say "You've never been big on that 'natural' look. If you decide you need to take a bath or go to bed, you can disrobe then; otherwise, keep 'em on.";
		rule fails.
...
The wedding ring is a thing. The indefinite article of the wedding ring is "your". The description of the wedding ring is "Staring at your wedding ring like that tends to make you sentimental.". The wedding ring is worn.
...
The pants are a plural-named proper-named opaque container with printed name "Michael's pants". "On the four-poster bed you can see Michael's pants." The description of the pants is "They're the same plain khakis he usually wears.". Instead of examining the pants, say "They're the same plain khakis he usually wears[if the pants contain something]. You notice something in the back pocket[end if]." instead.

The wallet is a proper-named opaque container with printed name "Michael's wallet". The wallet is in the pants. Instead of examining the wallet, say "It's just his wallet, an ordinary leather wallet." instead.
...
privileges that implies.". The faculty card is in the wallet.

Book 3 - Keys

The house key is a thing. "It's an ordinary-looking key. The word 'house' is written on a tiny piece of tape affixed to the key's tab." The house key unlocks the front door and the back door.

The cellar key is a thing. "It's an ordinary-looking key. The word 'cellar' is written on a tiny piece of tape affixed to the key's tab." The cellar key unlocks the cdoor.
...
The newspaper is a thing. "A damp newspaper lies on the curb, fluttering slightly in the wind." The description of the newspaper is "It's the 'Weekly Arkham Herald'. Anchorhead, apparently, is not large enough to warrant its own newspaper.". Instead of searching the newspaper, say "The front page story is about Jeffrey Greer, 8 years old, who was abducted from his home at #11 Mill
Town Road last night. Little Jeffrey is the latest victim in a series of abductions that stretches
back for years, one every six months or so, and that authorities believe is the work of a single
perpetrator. Local police had hoped to prove that Edward Verlac had been behind the kidnappings, but
were unable to obtain a confession or any hard proof. Edward Verlac was convicted of murdering his
wife and two daughters, one of whom was 15 months old, in January of this year; he was found not
guilty by reason of insanity and incarcerated in Danvers Asylum, where he remained until committing
suicide last March. This latest kidnapping, occurring after Edward's death, seems to have cleared up
any lingering suspicions that he might have been the culprit.

Anyone possessing information regarding the whereabouts of Jeffrey Greer is strongly urged to speak
to the authorities as soon as possible.".

Volume 6 - Random Rules
...
"You hear a soft rattle behind you, like an empty tin can rolling across the uneven ground. When you
turn, there is no one there."
...
The day1 intro is always "[bold type]November, 1997[roman type].

You take a deep breath of salty air as the first raindrops begin to spatter the pavement, and the swollen, slate-colored clouds that blanket the sky mutter ominous portents amongst themselves over the little coastal town of Informhead.
...
Dreamed of Father again. Dreamed of Grandfather. Those horrible, red-rimmed eyes...
 ...
 -- into the safe, finally. [m1 of safe]-[m2 of safe]-[m3 of safe]. Won't forget THAT soon. Ha!

CANNOT DISCOVER ENTRANCE IN THE CELLAR!!! Secret eludes me still but I WILL FIND IT!!! The clue is in their names, that pestilential procession of names! If I could only -- [roman type]

The text breaks off as several more pages are missing. the last fragmented entry reads:

[italic type] -- will fail. There is no recourse left. I know now what I must do. Julia -- [roman type]".
...
[italic type]...Father came again to my bed last night... mother doesn't... tells me I can't... to be a good daughter. Sometimes it hurts, but Father always tells me I shouldn't cry. Father says a daughter must do her Duty if she wants to get into Heaven. And I do want to get into Heaven...

(this entry dated 1957) [italic type]...my poor little William. Father calls him an aberration, child of the Devil, but I don't believe... locked in the attic. I go to see him whenever Father is away. I sing to him, sometimes, through the keyhole, and slip him sweets through the crack under the door... my baby is beautiful... can't let him hurt my dear baby William... to the doctor, and he has a plan... I can never... this locket, William, and I will always keep yours... to always remember my face...[roman type]
...
[italic type]...dead, but not dead yet... will not allow him to do to Edward what he wanted to do to William... have learned... given him the charm against the... never take it off, dear Edward...[roman type] (a page has been torn away) [italic type]...fear to sleep... mist at the window --[roman type]
